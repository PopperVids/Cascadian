package com.cascade.item;

import com.cascade.Cascade;
import net.fabricmc.fabric.api.item.v1.FabricItemSettings;
import net.minecraft.item.Item;
import net.minecraft.item.ItemGroup;
import net.minecraft.util.Identifier;
import net.minecraft.util.registry.Registry;

public class ModItems {

    public static final Item  RAW_CASCADE = registerItem("raw_cascade",
            new Item(new FabricItemSettings().group(ModItemGroup.CASCADE)));

    public static final Item  CASCADE = registerItem("cascade",
            new Item(new FabricItemSettings().group(ModItemGroup.CASCADE)));




    private static Item registerItem(String name, Item item) {
        return Registry.register(Registry.ITEM, new Identifier(Cascade.MOD_ID, name), item);
    }

    public static void registerModItems() {
        Cascade.LOGGER.debug("Registering Mod Items for" + Cascade.MOD_ID);
    }
}
